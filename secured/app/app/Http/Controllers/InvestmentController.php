...
class InvestmentController extends Controller
{
    ...
    public function newinv2(Request $request)
    {
         $request->validate([
            'id' => 'required|numeric',
//
        ], [
            'id.required' => 'Please select an investment plan',
        ]);

        $data['page_title'] = "New Investment Process";
        $user = Auth::user();
        $data['plan'] = Plan::find($request->id);
        $basic = GeneralSettings::first();
    	Session::put('Track', $request->id);

        return redirect()->route('previewinv');

    }

    public function previewinv()
    {
        $data['page_title'] = "Preview Investment Process";
        $user = Auth::user();
        $track = Session::get('Track');
        $data['plan'] = Plan::find($track);
        $data['plan2'] = $track;
        $basic = GeneralSettings::first();

        if($data['plan']->expiration == "One week")
        {
         $data['tenure'] = 7;
         }

        if($data['plan']->expiration == "Three weeks")
        {
         $data['tenure'] = 21;
         }
        if($data['plan']->expiration == "Four weeks")
        {
         $data['tenure'] = 28;
         }
       if($data['plan']->expiration == "Five weeks")
        {
         $data['tenure'] = 35;
         }
       if($data['plan']->expiration == "Four months")
        {
         $data['tenure'] = 112;
         }
        if($data['plan']->expiration == "Five months")
        {
         $data['tenure'] = 140;
         }
       if($data['plan']->expiration == "Six months")
        {
         $data['tenure'] = 168;
         }
        if($data['plan']->expiration == "Seven months")
        {
         $data['tenure'] = 196;
         }
        if($data['plan']->expiration == "Eight months")
        {
         $data['tenure'] = 224;
         }
        if($data['plan']->expiration == "Nine months")
        {
         $data['tenure'] = 252;
         }
        if($data['plan']->expiration == "Ten months")
        {
         $data['tenure'] = 280;
         }
        if($data['plan']->expiration == "Eleven months")
        {
         $data['tenure'] = 308;
         }
        if($data['plan']->expiration == "One year")
        {
         $data['tenure'] = 336;
         }

         if($data['plan']->increment_interval == "Hourly")
         {
          $data['cycle'] = $data['tenure'] * 24;
         }
         if($data['plan']->increment_interval == "Daily")
         {
          $data['cycle'] = $data['tenure'] * 1;
         }
         if($data['plan']->increment_interval == "Weekly")
         {
          $data['cycle'] = $data['tenure'] / 7;
         }
         if($data['plan']->increment_interval == "Monthly")
         {
          $data['cycle'] = $data['tenure'] / 30;
         }
...
//
    ...
    if($user->balance < $plan_price){
              return back()->withAlert('You dont have enough fund to purchase this inestment plan. Please deposit more fund and come back tomorrow for more');
    }

      if($plan_price < $plan->min_price){
              return back()->withAlert('You cant invest an amount lower than the minimum investment price');
    }
      if($plan_price > $plan->max_price){
              return back()->withAlert('You cant invest an amount greater than the maximum investment price');
      }

       if($user->balance >= $plan_price){

         $user->balance = $user->balance - $plan_price;
         $user->bonus = $user->bonus + $plan->gift;
         $user->save();
           }
        ...
        $basic = GeneralSettings::first();
        $earnings=$basic->refinv*$plan_price/100;
        $agent= User::where('id',$user->refer)->first();
           if(count($agent) > 0){
          $agent->bonus = $agent->bonus + $earnings;
     	  $agent->save();

     	    $basic = GeneralSettings::first();

        $to_name = $agent->username;
        $sitename = env('APP_NAME');
        $to_email = $agent->email;
        $heading = "Referral Earning";
        $data = array("name"=> $agent->username,"heading"=>"Referral Bonus Earning", "body" => "Your have successfully earned ".$basic->currency." ".$earnings." from ".$user->username."  who just purchase ".$plan->name." investment plan. Please keep sharing your affiliate link to earn more bonus. Thank you for choosing ".$basic->sitename."");
        ...
        $sitename = env('APP_NAME');
        $to_email = $auth->email;
        $heading = "New Investment Plan";
        $data = array("name"=> $auth->username,"heading"=>"New Investment", "body" => "Your have successfully joined ".$plan->name." investment plan. Your investment ID number is ".$trx.". Thank you for choosing ".$basic->sitename."");
      ...
      return redirect()->route('myplans')->with('message',"You successfully purchased a new investment plan and your investment plan is now active.");
    ...
    {
        $data['page_title'] = "My Investment Plans";
        $user = Auth::user();
        $data['active'] = User_plans::whereActive("yes")->whereUser($user->id)->get();
        $data['expired'] = User_plans::whereActive("expired")->whereUser($user->id)->get();
        $basic = GeneralSettings::first();

        return view('user.investment.myplans', $data);

    }
    public function viewmyplans($id)
    {
        $data['page_title'] = "My Investment Process";
        $user = Auth::user();
        $data['inv'] = User_plans::whereTrx($id)->whereUser($user->id)->first();
        $data['plan'] = Plan::whereId($data['inv']->plan)->first();
        $data['roi'] = Tp_transactions::wherePlan_id($data['inv']->trx)->whereUser($user->id)->get();
        $basic = GeneralSettings::first();

        return view('user.investment.myinv', $data);

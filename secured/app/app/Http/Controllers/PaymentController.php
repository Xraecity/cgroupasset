...
class PaymentController extends Controller
{
    ...
    public function userDataUpdate($data)
    {
        ...
        if ($data->status == 0) {
            ...
            $to_email = $auth->email;
            $heading = "New Deposit";
            $data = array("name"=> $auth->username,"heading"=>"New Deposit", "body" => $txt. " Thank you for choosing ".$basic->sitename."");
        ...
        if ($data->status != 0) {
         ...
         if ($request->bank) {
           ...
           if ($request->coin) {
            ...
            return redirect()->route('deposit')->with('success', 'Crypto Deposit Request Received.Please wait while our server vlidates your payment');
        }

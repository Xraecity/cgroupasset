...
//
            ...
            'otp.required' => 'Please enter your 2 Factor Authentication Password',
        ]);
            $user = User::find(Auth::user()->id);
            $valid = User::whereGfacode($request->otp)->count();
            if($valid < 1){
             return back()->with('danger', 'Your have entered an invalid 2FA Password. Please Try Again');
            }
            else{
            $timer = Carbon::now()->addMinutes(200);
       ...
       public function resendotp(Request $request)
    {
        ...
        $timer = Carbon::now()->addMinutes(120);
        ...
        $to_email = $auth->email;
        $heading = "2FA Password";
        $data = array("name"=> $auth->username,"heading"=>"2FA Password", "body" => "Your 2FA Password is ".$code.". Thank you for choosing ".$basic->sitename."");
        ...
        ->subject("2FA Password");
         ...
         session()->flash('success', '2FA Password Sent Successfully');
        return redirect()->route('home');
    }
    ...
    public function authCheck()
    {
        ...
        $user = User::find(Auth::user()->id);
       if(Auth()->user()->gfa == 1 && Auth()->user()->gfatime > Carbon::now()){
            $code = strtoupper(Str::random(6));
            $user->gfacode = $code;
            $user->save();
            $basic = GeneralSettings::first();

        session()->flash('danger', 'Please enter 2FA Password');
        return redirect()->route('home');
    ...
    public function sendEmailVcode(Request $request)
    {
        ...
        $data = array("name"=> $auth->username,"heading"=>"Verification Code", "body" => "Your verification code is ".$code.". Thank you for choosing ".$basic->sitename."");
...
//
  ...
  public function logtoggle(Request $request)
    ...
    public function deldepos($id)
    {
        $deposit = Deposit::whereId($id)->first();
        if($deposit->status == 0)
        {
          $deposit->delete();
           session()->flash('success', 'Deposit Cancelled Successfully. ');

         return redirect()->route('home');
        }
        else
        {
        return back()->with('danger', 'Deposit cant be deleted or canceled');
        }
...
$charge = $gate->fixed_charge + ($request->amount * $gate->percent_charge / 100);
    ...
    public function updateconvert(Request $request)
    {
       $basic = GeneralSettings::first();
        ...
        $data = array("name"=> $auth->username,"heading"=>"Bonus Converted", "body" => "Your have successfully converted your bonus walued at ".$basic->currency."".$request->amount.". Thank you for choosing ".$basic->sitename."");

        Mail::send("mail", $data, function($message) use ($to_name,  $heading, $to_email) {
        $message->to($to_email, $to_name,  $heading)
        ->subject("Bonus Converted");
 ...
 public function updatetransfer(Request $request)
    ...
    public function requestwithdrawal(Request $request)
    {
       ...
       if($request->method_id == 3){
        ...
        $sitename = env('APP_NAME');
        $to_email = $auth->email;
        $heading = "New Withtrawal";
        $data = array("name"=> $auth->username,"heading"=>"New Withrawal", "body" => "Your have successfully requested for a new withdrawal from your account. Amount Requested: ".$basic->currency."".$request->amount." Please wait while we verify your request. Thank you for choosing ".$basic->sitename."");

        Mail::send("mail", $data, function($message) use ($to_name,  $heading, $to_email) {
        $message->to($to_email, $to_name,  $heading)
        ->subject("New Withrawal");
    ...
    public function sellcoin()
    {
        ...
        $get['currency'] = Currency::whereStatus(1)->orderBy('name','desc')->get();
        $get['method'] = PaymentMethod::whereStatus(1)->orderBy('name','desc')->get();
        $get['bank'] = Bank::whereStatus(1)->orderBy('name','asc')->get();

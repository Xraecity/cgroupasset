...
class AdminController extends Controller
{
	 ...
	 public function investplans()
    {
        ...
        return view('admin.investment.index',$data);
    }
    	 public function activeinvplans()
    {
        $data['page_title'] = "Users Investment";
        $data['plans'] = User_plans::whereActive("yes")->orderBy('id', 'desc')->get();
        return view('admin.investment.requests',$data);
    }
    	 public function inactiveinvplans()
    {
        $data['page_title'] = "Users Investment";
        $data['plans'] = User_plans::whereActive("expired")->orderBy('id', 'desc')->get();
        return view('admin.investment.settled',$data);
    }
   	 public function terminateinv($id)
    {
        $data['page_title'] = "Users Investment";
        $plan = User_plans::whereId($id)->first();
        $plan->active = "expired";
        $plan->save();
         return redirect()->back()
        ->with('message', 'Investment Process Terminated Sucessfully!');
    }
   	 public function activateinv($id)
    {
        ...
        $plan->save();
         return redirect()->back()
        ->with('message', 'Investment Process Activated Sucessfully!');
    }

      public function invview($id)
    {
        $data['page_title'] = "My Investment Process";

        $data['inv'] = User_plans::whereTrx($id)->first();
        $data['plan'] = Plan::whereId($data['inv']->plan)->first();
        $data['roi'] = Tp_transactions::wherePlan_id($data['inv']->trx)->get();
        $basic = GeneralSettings::first();

        return view('admin.investment.details', $data);
     ...
     //Update plans
      public function updateplan(Request $request){
        ...
        return redirect()->back()
        ->with('message', 'Investment Plan Updated Sucessful!');
      }
         ...
         public function delplan($id)
    {
        ...
        $plan = Plan::whereId($id)->first();
         ...
         $plan->save();
        $plan->delete();
        $notification =  array('message' => 'Plan Deleted Successfully', 'alert-type' => 'success');
    ...
    public function addplan(Request $request){
      ...
      $plan->referral=$request['referral'];
    ...
    public function proex()
    {
        ...
        $data['page_title'] = 'Processed Exchange';
        return view('admin.currency.exchange-list', $data);
	}



    public function penex()
    {
        $data['exchange'] = Giftcardsale::where('status', '=',0)->latest()->get();
        $data['page_title'] = 'Pending Exchange';
        return view('admin.currency.exchange-list', $data);
	}



    public function decex()
    {
        $data['exchange'] = Giftcardsale::where('status', '=',2)->latest()->get();
        $data['page_title'] = 'Declined Exchange';
        return view('admin.currency.exchange-list', $data);
	}
    ...
    public function cardinfo($id)
    {
	  ...
	  public function approvegift($id)
    {
        $data = Giftcardsale::find($id);
        $data->status= 1;
        $data->save();
        ...
        Message::create([
                    ...
                    'details' => 'Your giftcard exchange with transaction number '.$data->trx.' was approved. Your fund has been credited into your wallet as requested',
                    'admin' => 1,
                    'status' =>  0
                ]);
	  ...
	  public function rejectgift($id)
    {
        ...
        Message::create([
                    ...
                    'details' => 'Your giftcard exchange with transaction number '.$data->trx.' was declined. Please feel free to open a support ticket if you feel this is wrong requested',
                    'admin' => 1,
                    'status' =>  0
                ]);
        ...
        $notification =  array('message' => 'Exchange Declined Successfully !!', 'alert-type' => 'success');
        return back()->with($notification);
	}
